import pandas as pd
def describir_datos_videos(data_frame_limpio):
    ...
    print("*** ESTADISTICAS ***")
    print(f"{data_frame_limpio[['id','usuario','calorias_quemadas','duracion_min']].describe()}")

    #Informacion de conteos valiosos
    print("*** CONTEOS ***")
    print(f"{data_frame_limpio['usuario'].value_counts()}")
    print(f"{data_frame_limpio['codigo'].value_counts()}")
    print(f"{data_frame_limpio['tipo_video'].value_counts()}")
